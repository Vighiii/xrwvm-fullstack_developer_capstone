...
def registration(request):
    ...
    if not all([username, password, first_name, last_name, email]):
        return JsonResponse(
            {"status": "Error", "message": "All five registration fields are required."},
            status=400,
        )

    if User.objects.filter(username__iexact=username).exists():
        return JsonResponse(
            {"userName": username, "status": "Already Registered"},
            status=409,
        )
...
def get_dealer_reviews(request, dealer_id: int):
    ...
    for review in reviews:
        item = dict(review)
        sentiment = analyze_review_sentiments(str(item.get("review", "")))
        item["sentiment"] = sentiment.get("sentiment", "neutral")
        enriched_reviews.append(item)
...
def add_review(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": 400, "message": "Dealer ID and car year must be numbers."},
            status=400,
        )
